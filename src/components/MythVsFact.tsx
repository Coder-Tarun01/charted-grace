...
const VisionBlock = ({ item, index }: { item: any; index: number }) => {
  ...
  return (
    <motion.div
      ...
      className="relative group cursor-crosshair h-[280px] md:h-[300px] rounded-[24px] overflow-hidden border border-slate-200 bg-white"
    >
      {/* Background Myth State (Blurred) */}
      <div className="absolute inset-0 p-5 flex items-center justify-center transition-all duration-700 bg-slate-50">
        <div className={`transition-all duration-1000 ${isHovered ? 'blur-2xl opacity-0 scale-95' : 'blur-0 opacity-100 scale-100'}`}>
          ...
          <h3 className="text-lg md:text-xl font-display font-bold text-slate-400 text-center max-w-sm mx-auto leading-tight italic">
            "{item.myth}"
          </h3>
        </div>
      </div>
      ...
      <motion.div
        ...
        className="absolute inset-0 bg-white z-10 p-4 md:p-5 flex flex-col items-center justify-center text-center shadow-2xl"
      >
        {/* Illustration */}
        <div className="w-full mb-3 flex items-center justify-center">
          <div className="w-full max-w-[120px] md:max-w-[135px] transform transition-transform duration-1000 group-hover:scale-105">
            {item.illustration}
          </div>
        </div>
...
const MythVsFact = () => {
  return (
    <section className="px-4 sm:px-6 lg:px-8 pt-8 md:pt-12 pb-12 md:pb-16 bg-white overflow-hidden">
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="text-base md:text-lg text-slate-500 max-w-2xl mx-auto leading-relaxed font-body font-light">
        ...
        {/* Discovery Grid */}
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
